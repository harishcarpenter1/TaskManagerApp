...
function App() {
  ...
  const handleCreateTask = () => {
    createTask({ variables: newTask, refetchQueries: [{ query: GET_TASKS }] });
    setNewTask({ title: '', description: '' });
  };

  const handleDeleteTask = id => {
    deleteTask({ variables: { id }, refetchQueries: [{ query: GET_TASKS }] });
  };

  const handleUpdateTask = (id, title, description) => {
    updateTask({ variables: { id, title, description }, refetchQueries: [{ query: GET_TASKS }] });
  };
